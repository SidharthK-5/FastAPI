...
class BookRequest(BaseModel):
    ...
    # class Config is a pydantic class which will help setting up a sample schema in Swagger UI
    class Config:
        json_schema_extra = {
            "example": {
                "title": "A new book",
...
BOOKS = [
    Book(
        1, "Computer Science Architecture", "Novice Coder", "A very nice book!", 5, 2030
    ),
    Book(2, "Be Fast with FastAPI", "Your guide to FastAPI", "A great book!", 5, 2030),
    Book(3, "Master Endpoints", "Youlearn", "A awesome book!", 5, 2029),
...
@app.get("/books/read/{book_id}", status_code=status.HTTP_200_OK)
...
async def update_book(book: BookRequest):
    ...
    if book_changed:
        ...
        raise HTTPException(
            status_code=404, detail=f"Book with ID {book.id} is not found"
        )
